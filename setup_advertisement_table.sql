...
-- Ensure advertisement_requests table exists
...
CREATE INDEX IF NOT EXISTS idx_advertisement_requests_ip_created_at ON advertisement_requests(user_ip, created_at);

-- Create index on created_at for admin dashboard
CREATE INDEX IF NOT EXISTS idx_advertisement_requests_created_at ON advertisement_requests(created_at DESC);

-- Add RLS policy (if using Row Level Security)
ALTER TABLE advertisement_requests ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist and recreate them
DROP POLICY IF EXISTS "Allow insert advertisement requests" ON advertisement_requests;
DROP POLICY IF EXISTS "Allow admin select advertisement requests" ON advertisement_requests;
DROP POLICY IF EXISTS "Allow admin delete advertisement requests" ON advertisement_requests;
